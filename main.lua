...
do
    ...
    if (F_A) then
        ...
        return Notify(nil, "Loaded", "wpfs admin is already loaded... use 'killscript' to kill", nil),
...
local WriteConfig = function(Destroy)
    ...
    if (isfolder("wpfs-admin") and Destroy) then
        delfolder("wpfs-admin");
        writefile("wpfs-admin/config.json", JSON);
        writefile("wpfs/admin/pluings/plugin-conf.json", PluginJSON);
    else
        makefolder("wpfs-admin");
        makefolder("wpfs-admin/plugins");
        makefolder("wpfs-admin/chatlogs");
        writefile("wpfs-admin/config.json", JSON);
        writefile("wpfs-admin/plugins/plugin-conf.json", PluginJSON);
    end
end

local GetConfig = function()
    if (isfolder("wpfs-admin")) then
        return JSONDecode(Services.HttpService, readfile("wpfs-admin/config.json"));
    else
        WriteConfig();
        return JSONDecode(Services.HttpService, readfile("wpfs-admin/config.json"));
    end
...
local GetPluginConfig = function()
    if (isfolder("wpfs-admin") and isfolder("wpfs-admin/plugins") and isfile("wpfs-admin/plugins/plugin-conf.json")) then
        local JSON = JSONDecode(Services.HttpService, readfile("wpfs-admin/plugins/plugin-conf.json"));
        ...
        WriteConfig();
        return JSONDecode(Services.HttpService, readfile("wpfs-admin/plugins/plugin-conf.json"));
    end
end
...
local SetPluginConfig = function(conf)
    ...
    writefile("wpfs-admin/plugins/plugin-conf.json", JSONEncode(Services.HttpService, NewConfig));
end

local SetConfig = function(conf)
    if (not isfolder("wpfs-admin") and isfile("wpfs-admin/config.json")) then
        WriteConfig();
    end
    ...
    writefile("wpfs-admin/config.json", JSONEncode(Services.HttpService, NewConfig));
end
...
AddCommand("commandcount", {"cc"}, "shows you how many commands there is in wpfs admin", {}, function(Caller)
...
AddCommand("dupetools2", {"rejoindupe"}, "sometimes a faster dupetools", {1,"1"}, function(Caller, Args)
    ...
    writefile("wpfs-admin/tooldupe.txt", tostring(Amount - 1));
    writefile("wpfs-admin/tooldupe.lua", format([[
        local OldPos = CFrame.new(%s);
        local DupeAmount = tonumber(readfile("wpfs-admin/tooldupe.txt"));
        local game = game
        local GetService = game.GetService
        ...
        if (DupeAmount <= 1) then
            ...
            delfile("wpfs-admin/tooldupe.txt");
            delfile("wpfs-admin/tooldupe.lua");
            loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/wpfsc/wpfs-admin/main/main.lua"))();
            ...
            writefile("wpfs-admin/tooldupe.txt", tostring(DupeAmount - 1));
            syn.queue_on_teleport(readfile("wpfs-admin/tooldupe.lua"));
...
AddCommand("esp", {"aimbot", "cameralock", "silentaim", "aimlock", "tracers"}, "loads wpfs esp", {}, function(Caller, Args, CEnv)
    loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/wpfsc/wpfs-esp/main/main.lua"))();
    return "esp enabled"
end)
...
AddCommand("trace", {"locate"}, "traces a player", {"1"}, function(Caller, Args)
    if (not EspLib) then
        EspLib = loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/wpfsc/wpfs-esp/main/esp-lib/esplibmain.lua"))();
    end
    local Target = GetPlayer(Args[1]);
...
AddCommand("globalchatlogs", {"globalclogs"}, "enables globalchatlogs", {}, function()
    ...
    if (not Socket) then
        Socket = (syn and syn.websocket or WebSocket).connect("ws://wpf0.xyz:8080/scripts/wpfs-admin/chat?username=" .. LocalPlayer.Name);
...
AddCommand("advertise", {}, "advertises the script", {}, function()
    local ChatRemote = Services.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest
    ChatRemote.FireServer(ChatRemote, "I am using wpfs admin, join the server 5epGRYR", "All");
end)

AddCommand("joinserver", {"discord"}, "joins the wpfs admin discord server", {}, function()
    ...
    if (Request({
        ...
        return "joined wpfs admin discord server"
    else
...
AddCommand("serverhop", {"sh"}, "switches servers (optional: min or max)", {{"min", "max"}}, function(Caller, Args)
    if (Caller == LocalPlayer) then
        ...
        if (#Servers >= 1) then
            ...
            if (syn) then
                syn.queue_on_teleport("loadstring(game.HttpGet(game, \"https://raw.githubusercontent.com/wpfsc/wpfs-admin/main/main.lua\"))()");
            end
...
AddCommand("changelogs", {"cl"}, "shows you the updates on wpfs admin", {}, function()
    local ChangeLogs = JSONDecode(Services.HttpService, game.HttpGetAsync(game, "https://api.github.com/repos/wpfsc/wpfs-admin/commits?per_page=100&path=main.lua"));
...
AddCommand("commands", {"cmds"}, "shows you all the commands listed in wpfs admin", {}, function()
...
AddCommand("reloadscript", {}, "kills the script and reloads it", {}, function(Caller)
    if (Caller == LocalPlayer) then
        ...
        loadstring(game:HttpGet("https://raw.githubusercontent.com/wpfsc/wpfs-admin/main/main.lua"))();
    end
end)
...
AddCommand("config", {"conf"}, "shows wpfs admin config", {}, function(Caller, Args, CEnv)
...
AddCommand("snipe", {"streamsnipe"}, "stream snipes a user", {"2"}, function(Caller, Args)
    ...
    local Ret = game.HttpGet(game, format("https://wpf123.000webhostapp.com/sniper.php?uid=%s&placeId=%s", UserId, PlaceId));
...
AddConnection(CConnect(ChatLogs.Save.MouseButton1Click, function()
    ...
    local String =  format("wpfs Admin Chatlogs for %s (%s)\n\n", GameName, os.date());
    local TimeSaved = gsub(tostring(os.date("%x")), "/", "-") .. " " .. gsub(tostring(os.date("%X")), ":", "-");
    local Name = format("wpfs-admin/chatlogs/%s (%s).txt", GameName, TimeSaved);
    local Children = GetChildren(ChatLogs.Frame.List);
    for i = 1, #Children do
        local v = Children[i]
        if (not IsA(v, "UIListLayout")) then
...
AddConnection(CConnect(HttpLogs.Save.MouseButton1Click, function()
    local Children = GetChildren(HttpLogs.Frame.List);
    local Logs =  format("wpfs Admin HttpLogs for %s\n\n", os.date());
    for i = 1, #Children do
        local v = Children[i]
        if (not IsA(v, "UIListLayout")) then
            Logs = format("%s%s\n", Logs, v.Text);
    ...
    if (not isfolder("wpfs-admin/httplogs")) then
        makefolder("wpfs-admin/httplogs");
    end
...
do
    ...
    Plugins = IsSupportedExploit and map(filter(listfiles("wpfs-admin/plugins"), function(i, v)
    ...
    if (PluginConf.PluginsEnabled) then
        ...
        if (IsSupportedExploit) then
            if (not isfolder("wpfs-admin") and not isfolder("wpfs-admin/plugins") and not isfolder("wpfs-admin/plugin-conf.json") or not isfolder("wpfs-admin/chatlogs")) then
                WriteConfig();
            end
        end
        ...
        AddCommand("refreshplugins", {"rfp", "refreshp", "reloadp"}, "Loads all new plugins.", {}, function()
            ...
            Plugins = map(filter(listfiles("wpfs-admin/plugins"), function(i, v)
                return lower(split(v, ".")[#split(v, ".")]) == "lua"
            end), function(i, v)
...
if (debug.info(2, "f") == nil) then
	Utils.Notify(LocalPlayer, "Outdated Script", "use the loadstring to get latest updates (https://wpfsc/wpfs-admin)", 10);
end
local LatestCommit = JSONDecode(Services.HttpService, game.HttpGetAsync(game, "https://api.github.com/repos/wpfsc/wpfs-admin/commits?per_page=1&path=main.lua"))[1]
wait(1);
